...
@app.get("/app_part1.js", include_in_schema=False)
def serve_app_part1():
    filepath = os.path.join(frontend_dir, "app_part1.js")
    with open(filepath, "rb") as f:
        content = f.read()
    return Response(content=content, media_type="application/javascript",
        headers={"Cache-Control": "no-store, no-cache", "Pragma": "no-cache"})

@app.get("/app_part2.js", include_in_schema=False)
def serve_app_part2():
    filepath = os.path.join(frontend_dir, "app_part2.js")
    with open(filepath, "rb") as f:
        content = f.read()
    return Response(content=content, media_type="application/javascript",
        headers={"Cache-Control": "no-store, no-cache", "Pragma": "no-cache"})
