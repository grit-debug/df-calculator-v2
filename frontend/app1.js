...
function renderFiyatListesi(urunMaliyet, baskiMaliyet) {
  ...
  FIYAT_KADEMELERI.forEach((k, i) => {
    const urunM5      = urunMaliyet * 1.05;
    const baskiM5     = baskiMaliyet * 1.05;
    const urunSatis   = urunM5 * (1 + k.marj);
    const baskiSatis  = isDTF ? baskiM5 * (1 + k.marj) : 0;
  ...
  if(baskiVar){
    if(baskiTip === 'sublimation'){
      ...
      if(dtfRefEn > 0 && dtfRefBoy > 0 && dtfRefFiyat > 0 && baskiAdedi > 0){
        const birimAlanFiyat = dtfRefFiyat / (dtfRefEn * dtfRefBoy);
        let satirlar = '';
